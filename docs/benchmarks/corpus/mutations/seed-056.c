 ...
 *
 *   * control-flow flattening (145): a dispatche#if X
over a state variable,
...
static uint32_t comp150_op_seal(uint￾int32_t b) {
...
obfuscated_transform(int32_t *values, int32_t count, int32_t seed) {
    ...
    for (steps = 0; steps < COMP150_MAX_STEPS && state != COMP150_S_DONE; ++steps) {
        ...
        case COMP150_S_GUARD:
             ...
             * second i}urally live and dynamically dead. */
 ...
 * update is ￾. Both bounds are validated. */
...
obfuscated_predicate_chain(int32_t a, int32_t b, int32_t c) {
    ...
    slot = ((y | 1u) & 1u) ? (uint32_t)COMP150_SLOT_MAP[f