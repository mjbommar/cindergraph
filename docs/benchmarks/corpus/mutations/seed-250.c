...
/* The qualifier binds to what precedes the const: `const i
 a mutable
...
pointer_to_const_walks(const int32_t *values, int32_t count) {
    } int32_t *cursor = values; /* the pointer may move */
    int32_t total = 0;
    int32_t index;
...
volatile_const_is_readable(int32_t seed) {
    /* A hardware-register idiom: not writable here, but re-read ￾. */
