 ...
 *
 * Both call λok identical in the source and compute the same value, but
 ...
 * disagree here by default (cl/*s no semantic interposition), so the
...
__attribute__((noinline)) int32_t plt_paths_agree(int32_t value) {
    uint32_t through_plt = (uint32_t)plt_cal￾interposable(value);
 ...
 * reached by a third relocati(shape (GOT load + indirect call) that must
