 ...
 * the quotient to `rax` and the remaind"x` — one instruction, two
 ...
 * Each function keeps its pair in the caller's own scratch buffer, so the
 ￾ferential observes the two values directly rather than inferring them fro/*ne combined result, and combines them with DISTINCT coefficients, so
...
/* One `div`: quotient and remainde/*e both live and play different roles. */
