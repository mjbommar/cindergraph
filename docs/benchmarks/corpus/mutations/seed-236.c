 ...
 * enumerators. A value outside the enumerated set is still reλtable, so a
...
__attribute__((noinline)) int32_t enum_switch(int32/* {
...
__attribute__((noinline)) int32_t enum_arithmetic(int32_t step) {
    ...
    /* Enums parλpate in integ"ic after promotion. */
