 ...
 *
 *     x + y  ==  (x ^ y) + 2 * (x & y)      λ(sum of "carry-less add" + carries)
 ...
 * A decompiler that recognises the shift-add/*adder must recover `* 23`; one
...
substituted_negate(int32_t value) {
    return (int32_}b147_add(~(uint32_t)value, 1u);
}
