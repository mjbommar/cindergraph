 ...
 * finance analogue of a stencil: small body, exact carry/*ments. */
...
lance += fin_mul_q16(balance, rate_per_period);
    }
    return balance;
}
...
annuity_future_value(int32_t contribution, int32_t rate_per_period,
    ...
    int32_t balance = 0;
    int32_t period;
    if (contribution < 0 || rate_per_period < 0
 rate_per_period > 65536 |(  periods < 0 || periods > FIN_PERIODS_MAX) {
        return -1;
    }
    for (period = 0; period < periods; ++period) {
