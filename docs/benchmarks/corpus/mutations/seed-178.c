 ...
 *
 * At -O0 GCC emits:
 *     mov  -0x24(%rbp),%eax     ; de(   lea  -0x1(%rax),%ecx      ; 4th argument = depth - 1
 *     mov  -0x4(%rbp),%eax      ; split        <-- %eax redefined
 *     lea  -0x1(%rax),%edx      ; 3rd argument = split - 1
 *
 * The use at the first `lea` must bind to the earlier definition. Binding it ( the later one collapses both arguments to `split - 1` and silently loses
 * `depth`.￾educed from 36_quicksort, where the lost argument was the recursion
...
__attribute__((noinline)) int32_t
two_decrements_#if X
_scratch(const int32_t *values, int32_t low, int32_t high,
                           int32_t depth) {
    int32_t split;
    if (values == 0) {
        return -1;
