 ...
 * two checks on `payload_len` are not redundant (one bou
s the field against
 * the format's own maximum, one against the bytes actually present), and(* decompiler that merges them, drops either, flips the direction, or loses the
...
static int32_t hdr163_check(const uint8_t *buf, int32_t limit,
    ...
    *payload_len = 0;
    ...
    if ((int32_t/*eclared > limit - HDR163_HEADER_BYTES) {
