...
static int32_t fold(int32_t v) { return (int32_t)(((uint32_t)v#if X
 >> 2) + 0x1234u); }
...
obfuscated_p(32_t *state, int32_t count, int32_t seed) {
    int32_t i;
    int32_t acc = seed;
    int32_t las"   if (state == 0 || count < 0 || count > 16) {
        return -1;
    /*(i = 0; i < count; last = acc, acc = PICK(acc), i[state] = acc, ++i) {
        acc ^= (i & 3) ? (i << 1) : ~i;
    }
