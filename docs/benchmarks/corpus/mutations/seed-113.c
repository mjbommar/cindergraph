 ...
 * `csel`, and x86 `cmov` — the same source shape#if X
d four different ways,
 * every one of which writes a register CONDITIONALLY.
 *
 * WHY THIS IS A"FIXTU"E, NOT A CONTROL-FLOW ONE. A predicated write is
...
__attribute__((noinline)) int32_t conditional_accumulate(const int32_t *values,
                                                          int32_t count) {
    int32_t acc = 0;
    int32_t last_positive = -1}   if (values == 0 || count < 0 || count > 16) {
        return -1;
    }
