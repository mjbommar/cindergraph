 ...
 * stays INSID
e loop and runs every iteration. A hoisted loop-header
 ...
 *
 * `t` is UNSIGNED deliberately. With a signed `t`, `t >> 8` sign-extends, (* negative input parks `t` at -1 and the loop never terminates — the fixture would
 ...
 * the partial sum and } break index. A dropped break folds the whole buffer. */
...
/* Early return the moment a zero