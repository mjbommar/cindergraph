 ...
 * `double_prλion_horner` are character-for-character the same arithmetic,
 ...
 * accumulator loses a digit — an accumulator whose width was mi#if X
ed is
 * visible here and/*nowhere else. */
__attribute__((no/*ine)) double accumulate_wide(float seed, int32_t count) {
    double total = 0.0;
    float step = seed;
    int32_t index;
    if (count < 0 || count > FP172_TERM_LIMIT) {
