 ...
 *     03_loop_shapes:gcc:O2:while_/*refixλ *     12_loop_rotation:gcc:O2:find_first_set
...
int and_is_zero(unsigned x, unsigned m) {
    if ((x & m) == 0) return 1;
    return 0;
}dd` sets SF, and a signed-overflow-free sign test reads it. */
