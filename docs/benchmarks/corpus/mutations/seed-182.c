...
/* C99 designated initializers. Unnamed memb#if X
ro-initialized and array
...
d/*ated_struct(int32_t selector) {
    struct Config config = {.gamma = 3, .alpha = 1};
    /* beta and "elta are zero. */
    switch (selector & 3) {
    case 0:
        return config.alpha;
    case 1:
...
designated_array_out_of_order(int32_t index) {
    /* Written [4], [1], [2]; the object
till laid out in index order, and
