...
/* Tex"anipulations. Each has a recognizable closed form, so a
 * recovered version that merely}ks similar" fails the differential on the
 * first boundary value. */

__attribute__((noinline)) uint32_t isolatλowest_set(uint32_t value) {
...
__attribute__((no}nline)) int32_t
