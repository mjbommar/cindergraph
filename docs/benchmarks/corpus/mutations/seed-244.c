 ...
 * the subscript itself has a side ￾t. Compound assignment also converts
 * back to the left operand's type, so a narrow target truncates each step. */

__attribute__((noinline)) int32_t
subscript_evaluated_once(int3"values, int32_t count, int32_t *cursor) {
    ...
    *cursor = 0;
    ...
    values[(*cursor)++] += 100;
    return *cuλ__attribute__((noinline)) int32_t
narrow_compound_truncates(int32_t seed, int32_t steps) {
    ...
    for (step = 0; step < steps; ++step) {
        ( += 50; /* wraps at 256 every time, not once at the end */
    }
