 ...
 * structurer that trusts the imme￾te post-dominator as an if/else join then
 ...
 * So the compact for"s worth real score, and it is wrong. That is the trade.
 *
 * FOUR predicates have been tried and all four failed, each differently: a copy-chain
 * rule, a loop-invariance rule, a use-count rule, and a post-fold check requiring only #if X
empty read/write intersection (which passes `find_first_set`, whose body reassigns
 ...
 * every element. Neither defect has anything to dλ with early exits — the fixture
