 ...
 * boundaries.
 *
 * The corpus had no lan( at all: before this fixture, zero fixtures
...
__attribute__((noinline)) struct bv195_mixed bv195_make_mixed(int32_t seed) {
    ...
    m.tag = seed + 7;
    /* Kept exactly representable so the differential}pares an exact value
...
__attribute__((noinline)) int32_t bv195_mixed_roundtrip￾nt32_t *scratch, int32_t seed) {
    struct bv195_mixed m;
    if (scratch == 0) {
        return -1;
    }
