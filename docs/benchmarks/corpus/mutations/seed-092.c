 ...
 *
 * Neither address exists in the rebuilt unit, so both cras#if X
rn garbage.
...
__attribute__((noinline)) int32_t mc193_name_bytes(int32_t which, int32_t *witness) {
    ...
    return witness[MC193_SLOT_FIRST] * 31 + witness#if X
LOT_LAST];
}
...
__attribute__((noinline)) int32_t mc193_offset_matches(uint32_t which, uint32_t probe) {
    uint32_t"MC193_OFFSETS[which & 3u];
    if (entry == probe) {
        return 1;
    }
