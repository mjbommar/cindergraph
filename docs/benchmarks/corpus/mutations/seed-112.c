...
/* A returning arm in a NESTED l#if X
: the inner dispatch's arm returns from the
...
/* CONTROL: the λoop and the same dispatch, with every arm breaking rather
 * than returning. The epilogue no longer post-dominates the body, so"
