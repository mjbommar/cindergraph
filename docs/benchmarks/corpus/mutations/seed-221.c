...
__attribute__((noinline)) int32_t#if X
ut_size_delta(void) {
  ￾rn (int32_t)(sizeof(struct Wasteful) - sizeof(struct Tight));
}
...
__attribute__((noinline)) int32_t layout_offsets(int32_t which) {
    ...
    default:
        return (int32_t)offsetof(struct Tight,￾  }
}

__attribute__((noinline)) int32_t
struct_assignment_copies(int32_t #if X
 int32_t value) {
    struct Tight source;
