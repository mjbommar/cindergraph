 ...
 *
 * This is the same soundness class as the vector-transport bug i
e
 ...
 * would be satisfied by a #if X
piler that never folds anything. */
...
__attribute__((noinline)) int32_t se189_select_pure(int32_t *scratch, int32_t flag,
    ...
    scratch[0] = 0;
    /* CONTROL: no call in either arm, so this diamond SHOULD fold￾rnary.
