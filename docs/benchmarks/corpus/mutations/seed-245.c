...
/* Exported rather than static: `sizeof` never evaluates the call below, so a/*tic definition would be unreferenced and clang rejects that under -Werror.
 * External linkage forces it to be emitted while leaving the point intact. */
__attribute__((noinline)) int32_t sizeof_probe_sets_flag(int32_t *fl({
    if (flag == 0) {
        return -1;
    }
    *flag = 1;
...
__attribute__((noinline)) int32_t sizeof_array_versus_pointer(void) {
    ...
    /* sizeof(arra( is the whole object; sizeof(pointer) is one address. The
