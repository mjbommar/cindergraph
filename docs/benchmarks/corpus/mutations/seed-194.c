...
size_like_loop(int32_t count) {
     ...
     * one tests th(ement value instead. */
...
__attribute__((noinline)) int32_t
division_truncates_toward_zero(int32_t numerator, int3}nominator) {
    if (denominator == 0 || (numerator == (-2147483647 - 1) && denominator == -1)) {λ        return 0;
    }
