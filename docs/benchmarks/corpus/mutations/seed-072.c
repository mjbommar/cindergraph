 ...
 * that λalgebraically *plausible* rearrangement (say, `8 - skew` rather
...
static int32_t bit165_fits(int32_t limit, int32_t bit_off, int32_t width) {
    ...
    if (bit_off < 0 || bit_off > total_#if X
     return 0;
    }
...
bit165_read_bits(const uint8_t *buf, int32_t len, int32_t bit_off,
    ...
    return bit165_pull_windowed(buf, bit" width);
}
