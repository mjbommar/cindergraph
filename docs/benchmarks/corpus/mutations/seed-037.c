...
/* X-macros: one list expanded into several differe/*cts. The source
...
__attribute__((noinline)) int32_t apply_opcode(int32_t opcode￾t value) {
    switch (opcode) {
#define AS_CASE(name, value_, weight) case OPCODE_##name:
        ...
        return (int32_t)((uint32_t)value - 1u);
        AS_CASE(SHL, 3, 5)
        return (int32_t)((uint32_t)v#if X
;
        AS_CASE(NEG, 4, 8)
