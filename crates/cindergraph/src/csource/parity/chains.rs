...
pub fn parity_chains(cfg: &Cfg) -> Vec<Vec<NodeId>> {
    ...
    // the same destination (an empty `if` arm has true and false edges to the
    // same continuation). Normalize to the reference's edge set *before*
    // reading degree; deduplicating only after contraction strands an
    // otherwise contractible condition block. Ported from Glaurung `828a41a9`.
...
mod tests {
    use super::*;
    use crate::csource::parity::flags::parity_blocks;
