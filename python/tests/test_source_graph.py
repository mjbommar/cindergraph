"""Graph export for C source, through `cindergraph.source` and the CLI.
...
NetworkX is not a dependency of this project -- `cindergraph.source_cfg` imports it
...
def test_every_representation_and_format_produces_a_named_graph(repr_name, format_name):
    graphs = cindergraph.source.export_graphs(GREET, repr=repr_name, format=format_name)
    assert len(graphs) == 1
    name, body = graphs[0]
    assert name == "greet"
    assert body.strip()
...
def test_an_unknown_repr_or_format_raises_rather_than_guessing():
    ...
    with pytest.raises(ValueError, match="cpg14"):
        cindergraph.source.export_graphs(GREET, repr="cpg14", format="dot")
    with pytest.raises(ValueError, match="graphson"):
        cindergraph.source.export_graphs(GREET, repr="cfg", format="graphson")
...
def test_export_is_total_on_input_that_is_not_c():
    ...
    for junk in ["", "\x00\x01 not C", "int f(", "}}}", "中文"]:
        for repr_name in cindergraph.source.EXPORT_REPRS:
            graphs = cindergraph.source.export_graphs(
                junk, repr=repr_name, format="dot"
            )
...
def test_a_partly_recovered_file_still_exports_what_parsed():
    ...
    names = [name for name, _ in cindergraph.source.export_graphs(text, repr="cfg")]
    assert "a" in names
...
def test_the_cfg_export_is_the_general_graph_not_the_parity_one():
    ...
    what would fail if this ever started reading `csource::parity`.
    """
    parity = cindergraph._native.csource.parity_cfgs(GREET)["greet"]
    body = dict(cindergraph.source.export_graphs(GREET, repr="cfg", format="json"))[
        "greet"
    ]
...
def test_graphml_round_trips_through_networkx_with_its_attributes():
    nx = pytest.importorskip("networkx")
    body = dict(cindergraph.source.export_graphs(GREET, repr="cfg", format="graphml"))[
        "greet"
    ]
...
def test_the_ast_export_is_a_tree():
    nx = pytest.importorskip("networkx")
    body = dict(cindergraph.source.export_graphs(GREET, repr="ast", format="json"))[
        "greet"
    ]
...
def test_export_path_reads_a_file_lossily(tmp_path: Path):
    ...
    graphs = cindergraph.source.export_path(target, repr="cfg", format="dot")
...
def test_mermaid_escapes_what_would_break_a_renderer():
    ...
    body = dict(cindergraph.source.export_graphs(text, repr="cfg", format="mermaid"))[
        "f"
    ]
...
def _cli(*args: str) -> subprocess.CompletedProcess:
    return subprocess.run(
        [sys.executable, "-m", "cindergraph.cli.main", "source-graph", *args],
        capture_output=True,
        text=True,
    )
...
def test_the_whole_fixture_corpus_exports_in_every_format():
    ...
    total = 0
    for path in sources:
        for format_name in cindergraph.source.EXPORT_FORMATS:
            graphs = cindergraph.source.export_path(
                path, repr="cfg", format=format_name
            )
            total += len(graphs)
