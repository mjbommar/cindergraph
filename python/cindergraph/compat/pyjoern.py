"""File-based C analysis behind the public cindergraph.source API.

Change ``from pyjoern import fast_cfgs_from_source`` to
``from cindergraph.source import fast_cfgs_from_source`` for graph-topology callers.
Install ``cindergraph[graphs]`` for NetworkX. No Joern executable is invoked.
...
def _networkx() -> ModuleType:
    try:
        ...
        raise ImportError(
            "Install cindergraph[graphs] to use NetworkX graphs"
        ) from error
    return nx
