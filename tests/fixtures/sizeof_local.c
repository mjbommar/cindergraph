/* Runtime oracle for named, fixed-size sizeof operands. */
#include <stddef.h>
...
int main(void) {
    int value = 17;
    if (scalar(0) != sizeof(int) || scalar(value) != sizeof(int)) return 1;
    if (pointer(NULL) != sizeof(int *) || pointer(&value) != sizeof(int *)) return 2;
    if (nested(0.0) != sizeof(double) || nested(3.5) != sizeof(double)) return 3;
